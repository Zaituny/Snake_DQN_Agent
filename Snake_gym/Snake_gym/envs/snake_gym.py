...
class SnakeEnv(gym.Env):
    ...
    def spawn_food(self, snake_pos, snake_body):
        new_location = [random.randrange(1, (self.frame_size_x//10)) * 10, random.randrange(1, (self.frame_size_y//10)) * 10]
        while True:
            if new_location[0] == snake_pos[0] and new_location[1] == snake_pos[1]:
                new_location = [random.randrange(1, (self.frame_size_x//10)) * 10, random.randrange(1, (self.frame_size_y//10)) * 10]
            for segment in snake_body:
                if new_location[0] == segment[0] and new_location[1] == segment[1]:
                    new_location = [random.randrange(1, (self.frame_size_x//10)) * 10, random.randrange(1, (self.frame_size_y//10)) * 10]
    ...
    def food_handler(self):
        ...
        if not self.food_spawn:
            self.food_pos = self.spawn_food(self.snake_pos, self.snake_body)
